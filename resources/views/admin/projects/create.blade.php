...
@section('title', __('messages.create') . ' ' . __('messages.Projects'))

@section('content')
<div class="d-flex justify-content-between align-items-center mb-4">
    <h1>{{ __('messages.create') }} {{ __('messages.Projects') }}</h1>
                ...
                <div class="col-md-6">
                    <div class="mb-3">
                        ...
                        <select class="form-control @error('type') is-invalid @enderror" id="type" name="type" required>
                            ...
                            <option value="1" {{ old('type') == '1' ? 'selected' : '' }}>{{ __('messages.done') }}</option>
                            <option value="2" {{ old('type') == '2' ? 'selected' : '' }}>{{ __('messages.on_going') }}</option>
                            <option value="3" {{ old('type') == '3' ? 'selected' : '' }}>{{ __('messages.planned') }}</option>
                        </select>
                        @error('type')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
