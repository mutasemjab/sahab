...
@section('title', __('messages.create') . ' ' . __('messages.PublicSessions'))

@section('content')
<div class="d-flex justify-content-between align-items-center mb-4">
    <h1>{{ __('messages.create') }} {{ __('messages.PublicSessions') }}</h1>
...
<div class="card">
    <div class="card-body">
        <form action="{{ route('public-sessions.store') }}" method="POST">
            @csrf
                ...
                <div class="col-md-4">
                    <div class="mb-3">
                        ...
                        <input type="time" class="form-control @error('from_time') is-invalid @enderror" 
                               id="from_time" name="from_time" value="{{ old('from_time') }}">
                        @error('from_time')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                ...
                <div class="col-md-4">
                    <div class="mb-3">
                        ...
                        <input type="time" class="form-control @error('to_time') is-invalid @enderror" 
                               id="to_time" name="to_time" value="{{ old('to_time') }}">
                        @error('to_time')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                ...
                <div class="col-md-6">
                    <div class="mb-3">
                        ...
                        <select class="form-control @error('type') is-invalid @enderror" id="type" name="type" required>
                            ...
                            <option value="1" {{ old('type') == '1' ? 'selected' : '' }}>{{ __('messages.open') }}</option>
                            <option value="2" {{ old('type') == '2' ? 'selected' : '' }}>{{ __('messages.soon') }}</option>
                        </select>
                        @error('type')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
            ...
            <div class="mb-3">
                ...
                <textarea class="form-control rich-text @error('what_expect') is-invalid @enderror" 
                          id="what_expect" name="what_expect" rows="4">{{ old('what_expect') }}</textarea>
                @error('what_expect')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
